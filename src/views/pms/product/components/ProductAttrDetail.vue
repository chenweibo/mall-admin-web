    ...
    <el-form ref="productAttrForm" :model="value" :rules="rules" label-width="120px" size="small">
      ...
      <el-form-item label="商品规格：">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <SkuUpload :url.sync="scope.row.pic" />
            </template>
          </el-table-column>
          <el-table-column
            label="商品库存"
        ...
        <el-card shadow="never" class="cardBg">
          <div v-for="(item,index) in selectProductParam" :key="index" :class="{littleMarginTop:index!==0}">
            ...
            <el-select v-if="item.inputType===1" v-model="selectProductParam[index].value" class="paramInput">
              <el-option
                v-for="i in getParamInputList(item.inputList)"
                :key="i"
                :label="i"
                :value="i"
              />
      ...
      <el-form-item label="规格参数：">
        <el-tabs v-model="activeHtmlName" type="card">
          <el-tab-pane label="电脑端详情" name="pc">
            <tinymce v-model="value.detailHtml" :width="900" :height="300" />
          </el-tab-pane>
          <el-tab-pane label="移动端详情" name="mobile">
            <tinymce v-model="value.detailMobileHtml" :width="900" :height="300" />
          </el-tab-pane>
        </el-tabs>
      </el-form-item>
